...
pub fn get_tasks_by_params_controller(params: &TaskParam) -> (i32, String, (Vec<Task>, TaskInfo)) {
    match Task::filter_tasks_by_params(params).0 {
        Ok(filtered_tasks) => (
            200,
            String::from("GET TASKS BY PARAMS OK"),
            (filtered_tasks, Task::filter_tasks_by_params(params).1),
        ),
        Err(e) => (204, e.to_string(), (Vec::new(), TaskInfo::new_empty())),
    }
}
