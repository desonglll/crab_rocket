...
#[get("/customer/<id>")]
pub fn get_customer_by_id(pool: &State<DbPool>, id: i32) -> Json<serde_json::Value> {
    crab_rocket_schema::update_reload::update_reload_count(pool);
    let resp = CustomerController::get_by_id(pool, id).unwrap();
    let json_value = serde_json::to_value(&resp).unwrap();
    Json(serde_json::from_value(json_value).unwrap())
}
...
pub fn insert_single_customer(
    pool: &State<DbPool>,
    customer: Json<PostCustomer>,
) -> Json<serde_json::Value> {
    let mut obj: PostCustomer = customer.into_inner();

    let resp = CustomerController::add_single(pool, &mut obj).unwrap();
    let json_value = serde_json::to_value(&resp).unwrap();
    Json(serde_json::from_value(json_value).unwrap())
}

#[delete("/customer/<id>")]
pub fn delete_customer_by_id(pool: &State<DbPool>, id: i32) -> Json<serde_json::Value> {
    let resp = CustomerController::delete_by_id(pool, id).unwrap();
    let json_value = serde_json::to_value(&resp).unwrap();
    Json(serde_json::from_value(json_value).unwrap())
}
...
pub fn update_customer_by_id(
    pool: &State<DbPool>,
    id: i32,
    task: Json<PatchCustomer>,
) -> Json<serde_json::Value> {
    let resp = CustomerController::update_by_id(pool, id, &task).unwrap();
    let json_value = serde_json::to_value(&resp).unwrap();
    Json(serde_json::from_value(json_value).unwrap())
}
