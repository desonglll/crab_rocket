...
pub fn get_suppliers(
    ...
    let resp = SupplierController::get_all(pool, &params).unwrap();
...
#[get("/supplier/<id>")]
pub fn get_supplier_by_id(pool: &State<DbPool>, id: i32) -> Json<serde_json::Value> {
    crab_rocket_schema::update_reload::update_reload_count(pool);
    let resp = SupplierController::get_by_id(pool, id).unwrap();
    let json_value = serde_json::to_value(&resp).unwrap();
    Json(serde_json::from_value(json_value).unwrap())
}
...
pub fn insert_single_supplier(
    pool: &State<DbPool>,
    supplier: Json<PostSupplier>,
) -> Json<serde_json::Value> {
    let mut obj: PostSupplier = supplier.into_inner();

    let resp = SupplierController::add_single(pool, &mut obj).unwrap();
    let json_value = serde_json::to_value(&resp).unwrap();
    Json(serde_json::from_value(json_value).unwrap())
}

#[delete("/supplier/<id>")]
pub fn delete_supplier_by_id(pool: &State<DbPool>, id: i32) -> Json<serde_json::Value> {
    let resp = SupplierController::delete_by_id(pool, id).unwrap();
    let json_value = serde_json::to_value(&resp).unwrap();
    Json(serde_json::from_value(json_value).unwrap())
}
...
pub fn update_supplier_by_id(
    pool: &State<DbPool>,
    id: i32,
    task: Json<PatchSupplier>,
) -> Json<serde_json::Value> {
    let resp = SupplierController::update_by_id(pool, id, &task).unwrap();
    let json_value = serde_json::to_value(&resp).unwrap();
    Json(serde_json::from_value(json_value).unwrap())
}
