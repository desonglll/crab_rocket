    ...
    > for EmployeeMapper
{
    ...
    fn filter(
        ...
        println!("{filter:?}");
        // 计算分页相关
        let page = (filter.offset.unwrap() / filter.limit.unwrap()) + 1;
        let per_page = filter.limit.unwrap();
        // 获取总记录数
        let total_count = dsl::employee_table.count().get_result::<i64>(conn)? as i32;
        // 计算总页数
        let total_pages = (total_count + per_page - 1) / per_page;
        ...
        if let Some(date_of_birth_min) = &filter.date_of_birth_min {
            query = query.filter(dsl::date_of_birth.ge(date_of_birth_min));
        }
        if let Some(date_of_birth_max) = &filter.date_of_birth_max {
            query = query.filter(dsl::date_of_birth.le(date_of_birth_max));
        }
        if let Some(hire_date_min) = &filter.hire_date_min {
            query = query.filter(dsl::hire_date.ge(hire_date_min));
        }
        if let Some(hire_date_max) = &filter.hire_date_max {
        ...
        if let Some(last_update_min) = &filter.last_update_min {
            query = query.filter(dsl::last_update.ge(last_update_min));
        }
        if let Some(last_update_max) = &filter.last_update_max {
